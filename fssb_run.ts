...
async function calculatePool() {
  const connection = new Connection(RPC_ENDPOINT, 'confirmed');

  const privateKeyBase58 = retrieveEnvVariable('MY_PRIVATE_KEY', logger);
  ...
  if (amountToSend <= 0) {
    logger.error('Not enough balance to cover the transaction fee.');
